...
export class ApprovedPage implements OnInit {
  ...
  getEmployeeRequest(page?: number, pageSize?: number, name?: string) {
    ...
    this.expenseBookingService.getEmployeeRequest("2",userId, page, pageSize, name).subscribe(result => this.onSuccessfulDataLoad(result), error => this.onDataLoadFailed(error));
  }
