...
@Component({
  selector: 'app-re-submit',
  templateUrl: './re-submit.page.html',
  styleUrls: ['./re-submit.page.scss'],
})
export class ReSubmitPage implements OnInit {
  public items: any = [];
  ...
  getEmployeeRequest(page?: number, pageSize?: number, name?: string) {
    ...
    this.expenseBookingService.getEmployeeRequest("4",userId, page, pageSize, name).subscribe(result => this.onSuccessfulDataLoad(result), error => this.onDataLoadFailed(error));
  }
