...
export class RejectedPage implements OnInit {
  ...
  getEmployeeRequest(page?: number, pageSize?: number, name?: string) {
    ...
    this.expenseBookingService.getEmployeeRequest("3",userId, page, pageSize, name).subscribe(result => this.onSuccessfulDataLoad(result), error => this.onDataLoadFailed(error));
  }
